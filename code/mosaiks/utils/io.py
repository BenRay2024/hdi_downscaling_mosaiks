...
def agg_polygon_X_y(c, c_app, X, Y_df, polygon_id_colname, weight_sum_col= False):
    ...
    X = Y_df.merge(X, # # Inner join so nonmatching labels and Xs are dropped
              on=["lat","lon"],
              how='inner'
       ).drop(columns=["lat","lon","id"])
    
    
    if pop_weight:
        total_pop_weights_colname = None
        if weight_sum_col:
            total_pop_weights_colname = "weight_sum"
            
        X = weighted_groupby(X, polygon_id_colname, c.pop_density_val_colname, total_pop_weights_colname)
    
    else:
        if weight_sum_col: #putting this up here for mem management. Don't want duplicate merges or duplicate full X dfs
            col = "X_0"
            sm_X = X[[polygon_id_colname, col]]
            counts = sm_X.groupby(polygon_id_colname)[col].count()
                         
        X = X.groupby(polygon_id_colname).mean() #Note that the index is now the polygon id
    
    y = X.pop(y_colname)
    
    if weight_sum_col and not pop_weight:
        X["weight_sum"] = counts
